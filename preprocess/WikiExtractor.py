...
class Extractor(object):
    ...
    def write_output(self, out, text):
        ...
        if options.write_json:
            ...
            # We don't use json.dump(preprocess, out) because we want to be
    ...
    def clean(self, text):
        ...
        if options.keep_tables:
            ...
            text = re.sub(r'!(?:\s)?style=\"[a-zа-я]+:(?:\d+)%;\"', r'', text)
            text = re.sub(r'!(?:\s)?style="[a-zа-я]+:(?:\d+)%;[a-zа-я]+:(?:#)?(?:[0-9a-zа-я]+)?"', r'', text)
            text = text.replace('|-', '')
            text = text.replace('|', '')
        if options.toHTML:
...
EXT_IMAGE_REGEX = re.compile(
    ...
    /([A-Za-zА-Яа-я0-9_.,~%\-+&;#*?!=()@\x80-\xFF]+)\.(gif|png|jpg|jpeg)$""",
    re.X | re.S | re.U)
...
class OutputSplitter(object):
    ...
    def __init__(self, nextFile, max_file_size=0, compress=True):
        ...
        :para compress: whether to write preprocess with bzip compression.
        """
