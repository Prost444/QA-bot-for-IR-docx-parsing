...
def is_valid_table(table: HtmlTable) -> bool:
    ...
    if table.num_cols < 1 or table.num_rows < 1:
        return False

    return True

def lists_to_tables(page_content, wiki_page):
    start = 0
    new_text = ""
    for lst in wiki_page.get_lists():
        new_text += page_content[start:lst.span[0]]
        new_text += '{|class="wikitable"\n'
        for item in lst.items:
            new_text += f'|{item}\n|-\n'
        new_text += "|}"
        start = lst.span[1]
    new_text += page_content[start:]
    return wtp.parse(new_text)
...
class TableExtractor(multiprocessing.Process):
    ...
    def extract(self, uuid, page_id, title, page_content) -> Iterator[Dict]:
        ...
        page_context = re.sub(r'\s\n', r'\n', page_context)
        wiki_page = wtp.parse(page_context)
        wiki_page = lists_to_tables(page_context, wiki_page)
        #if wiki_page.get_lists():
        #    print(wiki_page.get_lists())
...
def process():
    ...
    input_file = '/Users/aleksandrlazarev/Downloads/ruwiki-20241201-pages-articles-multistream.xml.bz2'
