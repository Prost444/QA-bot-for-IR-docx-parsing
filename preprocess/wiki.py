...
# В случае ошибки https://github.com/attardi/wikiextractor/commit/ab8988ebfa9e4557411f3d4c0f4ccda139e18875
from TaBERT.preprocess.WikiExtractor import pages_from, options, tagRE, Extractor, ignoreTag
from TaBERT.preprocess.htmltable import *
__DEBUG__ = False

os.environ['JAVA_HOME'] = 'C:\\Program Files\\Java\\jre1.8.0_431\\bin'
...
if __name__ == '__main__':
	input_file = Path('ruwiki-20241201-pages-articles-multistream1.xml-p1p224167.bz2')
	output_file = Path('wiki_dump.jsonl')

	job_queue = multiprocessing.Queue(maxsize=1000)
	example_queue = multiprocessing.Queue()
	num_workers = multiprocessing.cpu_count() - 1

	loader = multiprocessing.Process(target=data_loader_process, daemon=True, args=(input_file, job_queue, num_workers))
	loader.start()
	workers = []
	for i in range(num_workers):
	...
	writer = multiprocessing.Process(target=example_writer_process, daemon=True, args=(output_file, example_queue))
	writer.start()
